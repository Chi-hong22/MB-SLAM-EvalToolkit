...
%% === errorTimeSeries（误差时间序列模块） ===
    ...
    cfg.errorTimeSeries.saveData  = true;
    cfg.errorTimeSeries.truncateToCommonRange = true; % true 即取所有曲线中最短的最大时间作为横轴上限。
    ...
    cfg.errorTimeSeries.referenceDataset = struct( ...
        ...
        'insPath',      'Data/251111_NESP_noINS/NESP_noINS_seed20_yaw_0.05_0.005rad_overlap_coverage_0.6/poses_corrupted.txt');

    % benchmark 数据集列表（对比数据）
    ...
    % INS 配置（单条 INS 曲线，来源可配置）
    cfg.errorTimeSeries.ins = struct( ...
        'enable', true, ...
        'displayName', 'INS', ...
        'sourceDatasetId', 'comb');

    % 可视化配置
    % INS [223, 52, 52]/255;  % 深红 rgb(223, 52, 52)
    % Comb [ 0, 128, 255]/255;  % 深蓝 rgb(0, 128, 255)
    % A-BSLAM  [155, 92, 236]/255;  % 深紫 rgb(155, 92, 236)
    % NESP [ 64, 174, 97]/255;  % 深绿 rgb(64, 174, 97)
    % [217, 128,  38]/255;  % 深橙 rgb(217,128,38)
    cfg.errorTimeSeries.vis = struct();
    cfg.errorTimeSeries.vis.metricOrder = {'INS', 'Comb', 'A-BSLAM', 'NESP'};
    cfg.errorTimeSeries.vis.curves = [ ...
        struct('metricName', 'INS',  'color', [ 223, 52, 52]/255, 'lineStyle', cfg.global.visual.corrupted_line_style, 'lineWidth', cfg.global.visual.corrupted_line_width), ...
        struct('metricName', 'Comb', 'color', [ 0, 128, 255]/255, 'lineStyle', cfg.global.visual.optimized_line_style, 'lineWidth', cfg.global.visual.optimized_line_width), ...
        struct('metricName', 'A-BSLAM', 'color', [155, 92, 236]/255,               'lineStyle', '-',                                    'lineWidth', cfg.global.visual.optimized_line_width), ...
        struct('metricName', 'NESP', 'color', [ 64, 174, 97]/255,        'lineStyle', cfg.global.visual.gt_line_style,        'lineWidth', cfg.global.visual.gt_line_width) ...
    ];
...
%% === ape（APE 模块） ===
    ...
    cfg.cbee.paths.gt_pcd_dir       = 'Data/250911_Comb_noINS/submaps';
    cfg.cbee.paths.poses_original   = 'Data/250911_Comb_noINS/Comb_noINS_seed40_yaw_0.05_0.005rad_overlapcoverage_0.5/poses_original.txt';
    cfg.cbee.paths.poses_optimized  = 'Data/250911_Comb_noINS/Comb_noINS_seed40_yaw_0.05_0.005rad_overlapcoverage_0.5/poses_optimized.txt';
    cfg.cbee.paths.poses_corrupted  = 'Data/250911_Comb_noINS/Comb_noINS_seed40_yaw_0.05_0.005rad_overlapcoverage_0.5/poses_corrupted.txt';
    ...
    cfg.cbee.options.save_optimized_submaps     = false; % 是否将优化后的子地图持久化到磁盘
...
%% === loop（回环可视化模块） ===
    ...
    cfg.loop.paths.input_folder   = 'Data/260326_Ling_noINS/Ling_noINS_seed40_yaw_0.04_0.005rad_overlap_coverage_0.5'; 
